...
- No reaper task leaks after shutdown (``_reaper_task`` is done/cancelled).
...
from agent.lsp.manager import LSPService, REAPER_INTERVAL
...
# ---------------------------------------------------------------------------
# Unit tests for _reap_idle_clients
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_reap_idle_clients_removes_stale_client():
    """_reap_idle_clients must remove and shut down a client whose last-used
...
@pytest.mark.asyncio
async def test_reap_idle_clients_preserves_recent_client():
    """_reap_idle_clients must NOT remove a client used very recently."""
...
@pytest.mark.asyncio
async def test_reap_idle_clients_handles_missing_last_used():
    """_reap_idle_clients must reap a client with no last_used entry (treat as
...
@pytest.mark.asyncio
async def test_reaper_loop_reaps_idle_client_without_diagnostic(monkeypatch):
    ...
    # Patch REAPER_INTERVAL to a tiny value so the loop fires quickly.
    monkeypatch.setattr("agent.lsp.manager.REAPER_INTERVAL", 0.05)
    ...
    task = loop.create_task(svc._reaper_loop())
    svc._reaper_task = task
...
@pytest.mark.asyncio
async def test_shutdown_async_cancels_reaper_task():
    """_shutdown_async must cancel _reaper_task and await it so no task leak
