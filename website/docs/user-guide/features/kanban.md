...
- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_request_review`, `kanban_request_changes`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_attach`, `kanban_attach_url`, `kanban_attachments`, `kanban_create`, `kanban_link`, `kanban_unblock`, and the orchestrator-only `kanban_release_triage`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `hermes kanban`. `kanban_release_triage` requires a nonblank audit reason, derives its actor server-side, refuses live or dangling claims, and atomically lands triage work in `ready` or dependency-gated `todo`. It does not bypass dashboard authentication or mutate completed/blocked/running tasks. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
